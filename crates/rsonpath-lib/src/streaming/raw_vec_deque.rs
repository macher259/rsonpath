...
#[repr(transparent)]
struct BlockDeque(pub(crate) RefCell<VecDeque<ByteBlock>>);
...
impl<I: Iterator<Item = u8>> SliceSeekable for InnerStream<I> {
    ...
    fn is_member_match(&self, from: usize, to: usize, member: &JsonString) -> bool {
        ...
        if expected.len() < 16 {
            for (i, &b) in expected.iter().enumerate() {
                ...
                if deque.get(block_idx).map(|block| block[offset]) != Some(b) {
                    return false;
                }
            }
            ...
            if from == 0 {
                ...
                deque.get(block_idx).map(|block| block.0[offset]) != Some(b'\\')
            }
        } else {
            let len = deque.len() * BLOCK_SIZE;
            drop(deque);
    ...
    fn seek_backward(&self, from: usize, needle: u8) -> Option<usize> {
        let deque = self.data.0.borrow();
        let total_len = deque.len() * BLOCK_SIZE;
        ...
        loop {
            ...
            if deque.get(block_idx).map(|block| block.0[offset]) == Some(needle) {
                return Some(idx);
            }
    ...
    fn seek_forward<const N: usize>(&self, from: usize, needles: [u8; N]) -> Option<(usize, u8)> {
        ...
        loop {
            ...
            let b = deque.get(block_idx)?.0[offset];
...
impl<I: Iterator<Item = u8>> InnerStream<I> {
    ...
    fn new(iter: I) -> Self {
        ...
        Self {
            iter,
            data: BlockDeque(RefCell::new(VecDeque::with_capacity(lower_bound / BLOCK_SIZE))),
            released: 0,
        }
    }
    ...
    fn get_block(&mut self, idx: usize) -> Option<ByteBlock> {
        self.read_block(idx);
        self.data.0.borrow().get(idx).cloned()
    }

    #[inline(always)]
    fn read_block(&mut self, idx: usize) -> bool {
        while idx >= self.data.0.borrow().len() {
            ...
            next_block
                .into_iter()
                .for_each(|block| self.data.0.borrow_mut().push_back(ByteBlock(block)))
        }
        true
    }
    ...
    fn release_blocks(&mut self, last_idx_to_remove: usize) {
        ...
        self.data.0.borrow_mut().drain(0..=last_idx_to_remove);
    }
}
