...
impl<I: Iterator<Item = u8>> SliceSeekable for InnerStream<I> {
    ...
    fn is_member_match(&self, from: usize, to: usize, member: &JsonString) -> bool {
        ...
        for (i, &b) in expected.iter().enumerate() {
            ...
            if cached_block.map(|x: (usize, _)| x.0 != block_idx).unwrap_or(true) {
                cached_block = Some((block_idx, list.get(block_idx)));
            }

            if cached_block.map(|x| x.1).flatten().map(|block| block[offset]) != Some(b) {
                return false;
            }
        }
        ...
        if from == 0 {
            ...
            list.get(block_idx).map(|block| block.0[offset]) != Some(b'\\')
        }
    }
    ...
    fn seek_backward(&self, from: usize, needle: u8) -> Option<usize> {
        ...
        loop {
            ...
            if list.get(block_idx).map(|block| block.0[offset]) == Some(needle) {
                return Some(idx);
            }
    ...
    fn seek_forward<const N: usize>(&self, from: usize, needles: [u8; N]) -> Option<(usize, u8)> {
        ...
        loop {
            ...
            let b = list.get(block_idx)?.0[offset];
...
struct InnerStream<I: Iterator<Item = u8>> {
    iter: I,
    data: RefCell<SkipList<ByteBlock>>,
    released: usize,
}
...
impl<I: Iterator<Item = u8>> InnerStream<I> {
    ...
    fn new(iter: I) -> Self {
        ...
        Self {
            iter,
            data: RefCell::new(SkipList::with_capacity(lower_bound / BLOCK_SIZE)),
            released: 0,
        }
    }
